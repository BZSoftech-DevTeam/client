    ...
    const datasets = jsonData.flatMap((category) =>
      category.data.map((item, i) => ({
        ...
        backgroundColor: item.color
          ? `${item.color}80`
          : "rgba(0, 0, 255, 0.2)", // Lighter shade for background
        borderWidth: 1,
        fill: false,
      }))
    );
      ...
      {!fileData && !uploading ? (
        <div className="w-full mb-5">
          <label
            ...
            className="flex flex-col items-center justify-center py-9 w-full border border-gray-300 border-dashed rounded cursor-pointer bg-gray-50"
          >
      ...
      {fileData && !uploading && (
        <div>
          <button
            ...
            className="mb-4 px-4 py-2 bg-gradient-to-tl from-blue-600 to-violet-600 text-white rounded-sm"
          >
            Upload Again
          </button>
          ...
          <table className="min-w-full">
            <thead>
              <tr>
                <th className="px-4 py-2 bg-slate-100 border-slate-200 border">
                  Category
                </th>
                <th className="px-4 py-2 bg-slate-100 border-slate-200 border">
                  Label
          ...
          <div className="flex gap-4 mt-4 mb-4 ">
            <button
              ...
              className="px-4 py-2 bg-gradient-to-tl from-blue-600 to-violet-600 text-white rounded-sm"
            >
              Line Chart
            </button>
            <button
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.data.map((item, itemIndex) => (
                      ...
                      >
                        ...
                        <div
                          className="w-6 h-6 rounded-full border border-slate-200 relative cursor-pointer"
                          style={{ backgroundColor: item.color || "#0000FF" }}
                        >
                          <input
                            type="color"
